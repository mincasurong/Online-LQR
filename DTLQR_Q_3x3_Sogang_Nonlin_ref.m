...
while(1)
    ...
    j=1; h=0; ranksave = []; update = 0; detsave = [];
    figure('color','w');
    ...
    % Reference
    for k=1:2*Nt+1
        r(k) = 1*cos(2*pi*dt*(k-1))+0.1*cos(13.7*pi*dt*(k-1))+0.7*cos(2.7*pi*dt*(k-1));
    end
    ...
    while(1)
        ...
        if mod(i,N) == 0
            ...
            if norm(H-Hold) > epsilon
                if i>Tfinal/dt || norm(Xpi*Xpi') == inf || norm(Xpi*Xpi') == NaN , sprintf('Rank Error, Det = 0;'), break; end
