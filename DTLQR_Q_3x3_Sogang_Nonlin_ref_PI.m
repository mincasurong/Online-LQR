...
epsilon = 200; % Most important for convergence!! (Learning Stop)
Tfinal = 1; dt = 0.001; 
...
j=1; h=0; ranksave = []; update = 0; detsave = []; noise = 0; Weight_old = 10000;
u = zeros(Nt,1);
figure('color','w');
...
% Reference
for k=1:2*Nt+1
        r(k) = 1*cos(5*pi*dt*(k-1));
end
zbar_old = [X(1)^2; X(1)*X(2); X(1)*X(3); X(1)*-K*X; X(2)^2; X(2)*X(3); X(2)*-K*X; X(3)^2; X(3)*-K*X; (-K*X)^2];
...
%     r(:,i+1)=F*r(:,i);      % Next Trajectory
    ...
    % Target
    d_target=[X(:,i); u(i)]'*G*[X(:,i); u(i)];
    zbar_new=[X(1,i)^2; X(1,i)*X(2,i); X(1,i)*X(3,i); X(1,i)*u(i); X(2,i)^2; X(2,i)*X(3,i); X(2,i)*u(i); X(3,i)^2; X(3,i)*u(i); u(i)^2];
    zbar = zbar_old - gamma*zbar_new;
    zbar_old2 = zbar_old;
    zbar_old = gamma*zbar_new;
        % Target
...
%     zbar=[X(1,i)^2; X(1,i)*X(2,i); X(1,i)*X(3,i); X(1,i)*u(i); X(2,i)^2; X(2,i)*X(3,i); X(2,i)*u(i); X(3,i)^2; X(3,i)*u(i); u(i)^2];
    ...
    if mod(i,N) == 0
        ...
        update = 0;
        Weight = norm(H-Hold);
        if norm(H-Hold) > epsilon || Weight > Weight_old
            if norm(H-Hold) > epsilon, 1, end
            if Weight > Weight_old, 2, end
...
%            epsilon = norm(H-Hold)*10;
    ...
    %         drawnow;
    %     end
%     if i>2 , sprintf('Success'), break; end
